...
/*			else if (t.back() == ':')
	...
	for (auto i: req)
		if (known.count(i.second))
			...
			if (!_quiet)
				cout << "Unknown assembler token" << i.second << "at address" << i.first;
	...
	while (d != e)
	{
		...
		default:
		{
			...
			if (*d == '"')
			{
				...
				{
					if (!_quiet)
						cout << "String literal > 32 characters. Cropping.";
					s.resize(32);
				}
				h256 valHash;
				memcpy(valHash.data(), s.data(), s.size());
				...
				if (exec)
				{
					...
					if (compileLispFragment(d, e, _quiet, codes, locs, _vars) != -1)
					{
						...
						while (compileLispFragment(d, e, _quiet, codes, locs, _vars) != -1)
							if (!_quiet)
								cout << "Additional items in bare store. Ignoring.";
						bareLoad = false;
					}
				}
				...
				else
				{
					...
					if (it != c_instructions.end())
					{
						if (exec)
						{
							...
							{
								cout << "Expected " << (ea < 0 ? "at least" : "exactly") << abs(ea) << "arguments to operation" << t << "; got" << totalArgs << ".";
								break;
							}
					...
					else
					{
						...
						if (it != c_arith.end())
						{
							...
							if (!totalArgs)
							{
								cout << "Expected at least one argument to operation" << t;
								break;
							}
...
//								int i = (int)codes.size();
								if (totalArgs != 2)
								{
									cout << "Expected two arguments to binary operator" << t << "; got" << totalArgs << ".";
									break;
								}
								for (auto jt = codes.rbegin(); jt != codes.rend(); ++jt)
									appendCode(o_code, o_locs, jt->first, jt->second);
...
//									int i = (int)codes.size();
									if (totalArgs != 1)
									{
										cout << "Expected one argument to unary operator" << t << "; got" << totalArgs << ".";
										break;
									}
									for (auto it = codes.rbegin(); it != codes.rend(); ++it)
										appendCode(o_code, o_locs, it->first, it->second);
...
bytes eth::compileLisp(std::string const& _code, bool _quiet)
{
	char const* d = _code.data();
	char const* e = _code.data() + _code.size();
	bytes first;
